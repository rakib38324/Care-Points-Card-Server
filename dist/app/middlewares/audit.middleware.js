...
const auditLogs_1 = require("../models/auditLogs/auditLogs");
...
const getModelName = (req) => {
    const url = req.originalUrl;
    if (url.includes(`/${user_constent_1.USER_ROLE.admin}/`))
        return 'User';
    if (url.includes(`/${user_constent_1.USER_ROLE.superAdmin}/`))
        return 'User';
    if (url.includes(`/${user_constent_1.USER_ROLE.employer}/`))
        return 'EmployerApplication';
    if (url.includes(`/${user_constent_1.USER_ROLE.member}/`))
        return 'MemberApplications';
    if (url.includes(`/${user_constent_1.USER_ROLE.ngo}/`))
        return 'NGOApplication';
    if (url.includes(`/${user_constent_1.USER_ROLE.sponsor}/`))
        return 'SponsorApplications';
